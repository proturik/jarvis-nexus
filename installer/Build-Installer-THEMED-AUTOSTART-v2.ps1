...
function Fail([string]$Message) { throw "JARVIS fixed installer: $Message" }
...
function Assert-Parse([string]$Path) {
    ...
    if ($errors.Count -gt 0) { Fail "PowerShell syntax error in $Path at line $($errors[0].Extent.StartLineNumber)" }
}

$installer = $PSScriptRoot
$project = Split-Path -Parent $installer
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'release\JARVIS-NEXUS-ULTRA-Setup-THEMED-AUTOSTART-FIXED.exe' }
...
foreach ($path in @(
    (Join-Path $installer 'Install-Jarvis-CORE-v2.ps1'),
    (Join-Path $installer 'Install-Jarvis-THEMED-AUTOSTART-v2.ps1'),
    (Join-Path $installer 'Start-Jarvis-RELEASE.ps1'),
    (Join-Path $project 'windows-theme\Enable-Nexus-Desktop-v2.ps1')
)) { Assert-Parse (Need $path) }
foreach ($path in @((Join-Path $installer 'Install-Jarvis-THEMED-AUTOSTART-v2.cmd'), (Join-Path $installer 'Start-Jarvis-RELEASE.cmd'))) { Need $path | Out-Null }
...
$stage = Join-Path $WorkRoot ('FIXED-' + [guid]::NewGuid().ToString('N'))
...
# Strict clean allow-list. No user data or cloud-screen components are copied.
...
# The v2 desktop module is written under the stable public filename expected by the installer.
Copy-Item -LiteralPath (Need (Join-Path $project 'windows-theme\Enable-Nexus-Desktop-v2.ps1')) -Destination (Join-Path $themes 'Enable-Nexus-Desktop.ps1') -Force
...
foreach ($name in @('Install-Jarvis-THEMED-AUTOSTART-v2.cmd', 'Install-Jarvis-THEMED-AUTOSTART-v2.ps1', 'Install-Jarvis-CORE-v2.ps1')) { Copy-Item -LiteralPath (Join-Path $installer $name) -Destination $stage -Force }

$files = @('Install-Jarvis-THEMED-AUTOSTART-v2.cmd', 'Install-Jarvis-THEMED-AUTOSTART-v2.ps1', 'Install-Jarvis-CORE-v2.ps1', 'installer-manifest.json', 'payload.sha256', 'payload.zip')
...
$sed.Add("TargetName=$OutputPath"); $sed.Add('FriendlyName=JARVIS NEXUS ULTRA'); $sed.Add('AppLaunched=Install-Jarvis-THEMED-AUTOSTART-v2.cmd'); $sed.Add('PostInstallCmd=<None>'); $sed.Add('AdminQuietInstCmd=Install-Jarvis-THEMED-AUTOSTART-v2.cmd /quiet'); $sed.Add('UserQuietInstCmd=Install-Jarvis-THEMED-AUTOSTART-v2.cmd /quiet'); $sed.Add('SourceFiles=SourceFiles'); $sed.Add(''); $sed.Add('[Strings]')
...
$sedPath = Join-Path $stage 'JARVIS-NEXUS-FIXED.sed'; $sed | Set-Content -LiteralPath $sedPath -Encoding ascii
