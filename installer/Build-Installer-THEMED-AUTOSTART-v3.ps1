...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $project 'release\JARVIS-NEXUS-ULTRA-Setup-THEMED-AUTOSTART-TESTED.exe'
}
...
$stage = Join-Path $WorkRoot ('THEMED-AUTOSTART-TESTED-' + [guid]::NewGuid().ToString('N'))
...
# Stable installed name expected by the THEMED AUTOSTART wrapper.
...
# The visual wrapper expects the v2 filename. v3 core is deliberately staged under it.
...
$sedPath = Join-Path $stage 'JARVIS-NEXUS-THEMED-AUTOSTART-TESTED.sed'
