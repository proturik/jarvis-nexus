...
function Assert-CleanPayload([string]$PayloadRoot) {
    ...
    if ($null -ne $privateItem) {
        Fail "Private, backup, log, or forbidden mode content entered payload: $($privateItem.FullName)"
    }

    $files = Get-ChildItem -LiteralPath $PayloadRoot -Force -File -Recurse
    foreach ($file in $files) {
        $relative = Get-RelativeZipPath $PayloadRoot $file.FullName
        if (-not (Test-AllowedPayloadFile $relative)) {
            Fail "Payload file violates the exact allow-list: $relative"
        }
...
function Assert-ArchiveMatchesPayload([string]$PayloadRoot, [string]$ZipPath) {
    ...
    $expected = @(
        Get-ChildItem -LiteralPath $PayloadRoot -Force -File -Recurse |
        ForEach-Object { Get-RelativeZipPath $PayloadRoot $_.FullName } |
        Sort-Object
    )
    ...
    if ($null -ne $difference) {
        Fail "Archive entries differ from staged payload: $($difference | Out-String)"
    }
    foreach ($entry in $actual) {
        if (-not (Test-AllowedPayloadFile $entry)) {
            Fail "Archive entry violates the exact allow-list: $entry"
        }
    }
}
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $project 'release\JARVIS-NEXUS-ULTRA-Setup-THEMED-AUTOSTART-TESTED-v3.exe'
}
...
foreach ($path in @($outerPs1, $coreV3, $launcherPs1, $desktopV3, $syncV2)) {
    Assert-Parse $path
}
...
$stage = Join-Path $WorkRoot ('THEMED-AUTOSTART-TESTED-v3-' + [guid]::NewGuid().ToString('N'))
...
# Canonical ULTRA app payload: no .env, data, OMEGA, old theme adapters, or
# unused source variants are copied.
...
if ([regex]::Matches($index, [regex]::Escape($ultraScriptTag)).Count -ne 1) {
    Fail 'Expected exactly one ultra.js script tag in staged index.html.'
}
if ($index.Contains($wakeScriptTag)) {
    Fail 'Staged index.html unexpectedly already contains the wake patch tag.'
}
...
if ($verifiedIndex.IndexOf($wakeScriptTag) -le $verifiedIndex.IndexOf($ultraScriptTag)) {
    Fail 'Wake patch script was not injected immediately after ultra.js.'
}
...
[ordered]@{
    ...
    client = 'ultra-v2.js with the JARVIS wake-word patch v3'
...
# The outer v3 wrapper deliberately retains this stable filename.
Copy-Item -LiteralPath $coreV3 -Destination (Join-Path $stage 'Install-Jarvis-CORE-v2.ps1') -Force

$files = @(
    'Install-Jarvis-THEMED-AUTOSTART-v3.cmd',
    'Install-Jarvis-THEMED-AUTOSTART-v3.ps1',
...
$sedPath = Join-Path $stage 'JARVIS-NEXUS-THEMED-AUTOSTART-TESTED-v3.sed'
