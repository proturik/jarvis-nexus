...
function Assert-NodeSyntax([string]$Node, [string]$Path) {
    ...
    if ($null -eq $exitCodeVariable -or [int]$exitCodeVariable.Value -ne 0) { Fail "Node syntax check failed: $Path" }
}
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'release\JARVIS-NEXUS-ULTRA-Setup-THEMED-AUTOSTART-TESTED-v6.exe' }
...
$stage = Join-Path $WorkRoot ('THEMED-AUTOSTART-TESTED-v6-' + [guid]::NewGuid().ToString('N'))
...
New-Item -ItemType Directory -Path $app, $runtime, $launcher, $desktopShell, $publicUltra, $themes -ErrorAction Stop | Out-Null
...
$serverText = Replace-Exactly $serverText '    reply = await askCloud(message);' ("    reply = await askLocalBrain(message);" + [Environment]::NewLine + '    if (reply === null) reply = await askCloud(message);') 'local-first chat route'
...
$launcherText = Get-Content -LiteralPath $launcherPs1 -Raw
$petLaunchBlock = @'
$PetPath = Join-Path $InstallRoot 'desktop-shell\JarvisPet.exe'
if (-not (Test-Path -LiteralPath $PetPath -PathType Leaf)) { throw "JARVIS desktop pet missing: $PetPath" }
$petAlreadyRunning = Get-Process -Name 'JarvisPet' -ErrorAction SilentlyContinue | Select-Object -First 1
if ($null -eq $petAlreadyRunning) {
    Start-Process -FilePath $PetPath -WorkingDirectory (Split-Path -Parent $PetPath)
}

'@
$launcherText = Replace-Exactly $launcherText '$edgeCandidates = @(' ($petLaunchBlock + '$edgeCandidates = @(') 'desktop pet launch insertion'
Set-Content -LiteralPath $stagedLauncher -Value $launcherText -Encoding utf8
Assert-Parse $stagedLauncher
$stagedLauncherCmd = Join-Path $launcher 'Start-Jarvis-RELEASE.cmd'
$launcherCmdText = Get-Content -LiteralPath $launcherCmd -Raw
...
Set-Content -LiteralPath $stagedLauncherCmd -Value $launcherCmdText -Encoding ascii

$stagedCore = Join-Path $stage 'Install-Jarvis-CORE-v2.ps1'
$coreText = Get-Content -LiteralPath $coreV3 -Raw
...
$coreText = Replace-Exactly $coreText "    `$launcher = Join-Path `$installRoot 'launcher\Start-Jarvis-RELEASE.cmd'" ("    `$launcher = Join-Path `$installRoot 'launcher\Start-Jarvis-RELEASE.cmd'" + [Environment]::NewLine + "    `$petPath = Join-Path `$installRoot 'desktop-shell\JarvisPet.exe'") 'core pet verification path'
$coreText = Replace-Exactly $coreText 'foreach ($required in @($serverPath, $nodePath, $launcher)) {' 'foreach ($required in @($serverPath, $nodePath, $launcher, $petPath)) {' 'core pet verification list'
Set-Content -LiteralPath $stagedCore -Value $coreText -Encoding utf8
...
[ordered]@{
    ...
    privacy = 'Exact archive allow-list; no .env, data, chats, profile, memory, tasks, events, logs, backups, API credentials, or model files.'
    client = 'ultra-v2.js with wake patch v3, JARVIS desktop pet, and optional local Ollama runtime'
    localBrain = 'llama3.1:8b is the optional default download; llama3.2 is an optional fast fallback; neither model is bundled.'
...
function Assert-OllamaExit([string]$Operation) {
    $exitCodeVariable = Get-Variable -Name LASTEXITCODE -ErrorAction SilentlyContinue
    if ($null -ne $exitCodeVariable -and [int]$exitCodeVariable.Value -ne 0) { throw "$Operation exited with code $($exitCodeVariable.Value)." }
}
...
# The default smart local model is about 5 GB and is not included in the EXE.
...
if (Confirm-LocalBrain 'Optionally download llama3.2 as a smaller, faster local fallback? It is not required.' 'JARVIS Local Brain') {
...
$localBrainTag = '            <CheckBox x:Name="SetupLocalBrain" IsChecked="True" Content="Set up Local Brain (Ollama, default about 5 GB)" Margin="2,4" Foreground="#DDFBFF"/>'
...
$sedPath = Join-Path $stage 'JARVIS-NEXUS-THEMED-AUTOSTART-TESTED-v6.sed'
