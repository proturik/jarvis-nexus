...
$ReleaseId = 'themed-autostart-v9'

function Fail([string]$Message) { throw "JARVIS v9 installer: $Message" }
...
function Assert-V9Stage([string]$PayloadRoot, [string]$ServerPath, [string]$LauncherPath, [string]$CorePath, [string]$OuterPath, [string]$LocalBrainPath) {
    ...
    if ($server -notmatch "provider: 'local'") { Fail 'Local provider is not the v9 default.' }
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $project 'release\JARVIS-NEXUS-ULTRA-Setup-THEMED-AUTOSTART-v9.exe' }
...
$stage = Join-Path $WorkRoot ('THEMED-AUTOSTART-v9-' + [guid]::NewGuid().ToString('N'))
...
$localBrainTag = '            <CheckBox x:Name="SetupLocalBrain" IsChecked="False" Content="Optionally set up Local Brain (Ollama, about 5 GB)" Margin="2,4" Foreground="#DDFBFF"/>'
...
$sedPath = Join-Path $stage 'JARVIS-NEXUS-THEMED-AUTOSTART-v9.sed'
