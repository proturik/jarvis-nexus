...
function Assert-OptionalNativeExitCode([string]$Operation) {
    $exitCodeVariable = Get-Variable -Name LASTEXITCODE -ErrorAction SilentlyContinue
    if ($null -ne $exitCodeVariable -and [int]$exitCodeVariable.Value -ne 0) {
        throw "$Operation stopped with code $($exitCodeVariable.Value)."
    }
}

function Run-CoreInstall {
    & $InstallCore -Quiet
    Assert-OptionalNativeExitCode 'Installation core'
}
...
function Enable-NexusDesktop {
    ...
    Assert-OptionalNativeExitCode 'NEXUS desktop module'
}
...
function Enable-JarvisAutostart {
    ...
    $shortcut.Arguments = '-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File ' + $quote + $startScript + $quote
...
'@
    ...
    $applyTheme = $window.FindName('ApplyTheme')
    $enableAutostart = $window.FindName('EnableAutostart')
    ...
    $installButton.Add_Click({
        ...
        try {
            Run-CoreInstall
            if ([bool]$applyTheme.IsChecked) {
                $status.Text = 'APPLYING SAFE NEXUS DESKTOP...'
                Enable-NexusDesktop
            }
            if ([bool]$enableAutostart.IsChecked) {
                $status.Text = 'ARMING LOCAL AUTOSTART...'
                Enable-JarvisAutostart
            }
