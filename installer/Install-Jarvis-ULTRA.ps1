...
function Get-InstallerManifest {
    ...
    if (-not $manifest.payloadSha256 -or -not $manifest.version -or $manifest.entrypoint -ne 'ultra-server.mjs') {
        throw "$ProductName installer manifest is invalid or targets an unsupported mode."
    }
    return $manifest
}
...
function Test-PayloadIntegrity {
    ...
    if ($actual -ne $expected) {
        throw "$ProductName payload integrity check failed."
    }
}
...
function Start-Ultra {
    param([string]$InstallRoot)

    $launcher = Join-Path $InstallRoot 'Start-Jarvis-ULTRA.ps1'
...
function Invoke-UltraInstallation {
    param(
    ...
    $extractionRoot = Join-Path $InstallerRoot ('JNU-ultra-' + [guid]::NewGuid().ToString('N'))

    & $OnStatus 'Готовлю установку только для текущего пользователя…'
    ...
    $packageUltra = Join-Path $packageApp 'ultra-server.mjs'
    $packageNode = Join-Path $packageRuntime 'node.exe'

    foreach ($required in @($packageApp, $packageRuntime, $packageLauncher, $packageUltra, $packageNode)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "$ProductName package safety check failed: missing $required"
        }
    }
    if (Test-Path -LiteralPath (Join-Path $packageApp 'data') -PathType Container) {
    ...
    if (Test-Path -LiteralPath (Join-Path $packageApp 'server.mjs') -PathType Leaf) {
        throw "$ProductName package safety check failed: only ultra-server.mjs is allowed."
    }

    & $OnStatus 'Обновляю ULTRA-ядро, не трогая память и настройки…'
    ...
    if ($CreateDesktopShortcut) {
        ...
        $result.DesktopShortcutCreated = New-UltraShortcut -Path $desktopLink -Launcher $launcher -WorkingDirectory $installRoot
    }
    if ($CreateStartupShortcut) {
        $startup = [Environment]::GetFolderPath([Environment+SpecialFolder]::Startup)
        $startupLink = Join-Path $startup "$ProductName.lnk"
        $result.StartupShortcutCreated = New-UltraShortcut -Path $startupLink -Launcher $launcher -WorkingDirectory $installRoot
    }
...
function Show-UltraInstallerWindow {
    $manifest = Get-InstallerManifest
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Grid ClipToBounds="True">
    ...
    <Ellipse Width="360" Height="360" Fill="#194E4BFF" Opacity="0.18" HorizontalAlignment="Right" VerticalAlignment="Bottom" Margin="0,0,-125,-155">
...
'@
    ...
    $script:ultraInstalled = $false
    $script:ultraInstallRoot = $null
    $installButton.Add_Click({
        if ($script:ultraInstalled) {
            Start-Ultra -InstallRoot $script:ultraInstallRoot
            $window.Close()
            return
        }
        ...
        $operation = [Action]{
            try {
                $result = Invoke-UltraInstallation -CreateDesktopShortcut ([bool]$desktopShortcut.IsChecked) -CreateStartupShortcut ([bool]$startupShortcut.IsChecked) -OnStatus {
                    param($message)
                    $status.Text = $message
                }
                $script:ultraInstalled = $true
                $script:ultraInstallRoot = $result.InstallRoot
...
if ($Quiet) {
    $result = Invoke-UltraInstallation -CreateDesktopShortcut $true -CreateStartupShortcut $false -OnStatus {
        param($message)
