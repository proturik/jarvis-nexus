...
foreach ($name in @('Build-Installer-RELEASE.ps1', 'Install-Jarvis-RELEASE.ps1', 'Start-Jarvis-RELEASE.ps1')) {
...
foreach ($name in @('Install-Jarvis-RELEASE.cmd', 'Start-Jarvis-RELEASE.cmd')) {
    if (-not (Test-Path -LiteralPath (Join-Path $installerRoot $name) -PathType Leaf)) { $errors.Add("Missing $name") }
}
...
Write-Host 'JARVIS NEXUS RELEASE installer preflight passed.' -ForegroundColor Cyan
