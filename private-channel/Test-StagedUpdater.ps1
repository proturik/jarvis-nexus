...
try {
    ...
    Assert-Fails { & (Join-Path $PSScriptRoot 'Invoke-JarvisStagedUpdate.ps1') -EnvelopePath $unmarkedCurrentEnvelope -PackagePath $unmarkedCurrentPackage -InstallRoot $unmarkedDir -CurrentVersion '1.0.0' -StateRoot $stateRoot -Activate } 'Program directory marker is missing'
    ...
    Assert-Fails { & (Join-Path $PSScriptRoot 'Invoke-JarvisStagedUpdate.ps1') -EnvelopePath $unmarkedPayloadEnvelope -PackagePath $unmarkedPayloadPackage -InstallRoot $install -CurrentVersion '1.0.0' -StateRoot $stateRoot -Activate } 'Program directory marker is missing'
