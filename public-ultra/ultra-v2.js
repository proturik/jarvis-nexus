...
function actionCard(action) {
  ...
  detail.textContent = action.risk === 'sensitive'
    ? `${action.detail} Для этой команды нужно подтверждение на панели.`
    : `${action.detail} Скажи «подтверждаю» или нажми кнопку.`;
  copy.append(title, detail);
  const button = document.createElement('button');
  button.type = 'button';
  button.textContent = action.risk === 'sensitive' ? 'ПОДТВЕРДИТЬ НА ПАНЕЛИ' : 'ПОДТВЕРДИТЬ';
  button.addEventListener('click', () => showConfirmation(action));
...
function speak(text) {
  ...
  const spokenText = text.replace(/\[.*?\]/gu, '').trim();
  if (!spokenText) return;
  const now = Date.now();
  if (spokenText === lastSpoken && now - lastSpokenAt < 2500) return;
  lastSpoken = spokenText; lastSpokenAt = now;
...
function isVoiceConfirmation(text) {
  const normalised = text.trim().replace(/[.!?]+$/u, '');
  return /^(?:подтверждаю(?:\s+команду)?|выполняй|давай\s+выполняй|подтвердить)$/iu.test(normalised);
}

async function send(forced = '', source = 'text') {
  const text = (forced || ui.commandInput.value).trim();
  if (!text || sending) return;
  if (source === 'voice' && pendingAction && isVoiceConfirmation(text)) {
    message('user', text);
    if (pendingAction.risk === 'sensitive') {
      showConfirmation(pendingAction);
      const warning = 'Эта команда чувствительная, поэтому подтверждение оставил на панели.';
      message('assistant', warning); speak(warning);
    } else {
      await confirm();
    }
    return;
  ...
  try {
    ...
    if (result.action) {
      ...
      speak(result.action.risk === 'sensitive'
        ? 'Команда ждёт подтверждения на панели.'
        : 'Команда готова. Скажи «подтверждаю», чтобы выполнить.');
...
function initialiseVoice() {
  ...
  if (!Recognition) { ui.voiceKey.disabled = true; ui.commandHelp.textContent = 'В этом приложении голосовой ввод недоступен. Текстовый контур работает полностью.'; return; }
  recognition = new Recognition(); recognition.lang = 'ru-RU'; recognition.interimResults = true; recognition.continuous = false;
  recognition.onstart = () => { finalTranscript = ''; listening = true; ui.voiceKey.classList.add('is-live'); setCore('listening', 'СЛУШАЮ ТЕБЯ'); ui.commandHelp.textContent = 'Слушаю. Говори нормально, шеф.'; };
  recognition.onresult = (event) => { let interim = ''; for (let index = event.resultIndex; index < event.results.length; index += 1) { const phrase = event.results[index][0].transcript; if (event.results[index].isFinal) finalTranscript += `${finalTranscript ? ' ' : ''}${phrase}`; else interim += phrase; } ui.commandInput.value = finalTranscript || interim; };
  recognition.onerror = (event) => { if (event.error !== 'aborted') toast(`Голосовой контур: ${event.error}`, true); };
  recognition.onend = () => { const transcript = finalTranscript.trim(); finalTranscript = ''; listening = false; ui.voiceKey.classList.remove('is-live'); if (!speechSynthesis.speaking) setCore(); ui.commandHelp.textContent = 'Скажи команду, затем «подтверждаю» — для обычных действий JARVIS выполнит её голосом.'; if (transcript) send(transcript, 'voice'); };
}
...
async function confirm() {
  ...
  try {
    const result = await api('/api/actions/execute', { method: 'POST', body: JSON.stringify({ token: action.token }) });
    ui.confirmDialog.close(); message('assistant', result.message); toast(result.message); speak('Готово.'); await refresh();
  } catch (error) {
    ui.confirmDialog.close(); const failure = `Команда не выполнена: ${error.message}. Скажи её заново.`;
    message('assistant', failure); toast(error.message, true); speak('Команда не выполнена. Скажи её заново.');
...
function installEvents() {
  ...
  ui.voiceKey.addEventListener('click', () => { if (!recognition) return; if (listening) recognition.stop(); else { speechSynthesis.cancel(); try { recognition.start(); } catch { /* browser is restarting speech recognition */ } } });
