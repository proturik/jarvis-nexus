(() => {
  ...
  function stopWake() {
    ...
    if (help) {
      help.textContent = 'Голосовой режим выключен. Нажми микрофон, затем скажи: «JARVIS, открой блокнот».';
    }
  }
  ...
  function initialiseWake() {
    ...
    recognizer.onend = async () => {
      ...
      if (command) {
        ...
        if (help) {
          help.textContent = 'Сначала скажи «JARVIS», затем команду. Например: «JARVIS, открой блокнот».';
        }
      } else if (help) {
